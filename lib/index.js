...
MusicMetadata.prototype.readEvent = function (event, value) {
  // only emit aliased events once the 'done' event has been raised,
  // because an alias like 'artist' could have values split over many data chunks.
  if (event === 'done') {
    ...
    // don't emit the metadata event if nothing ever gets added to the metadata object
